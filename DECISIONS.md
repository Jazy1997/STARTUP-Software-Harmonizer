...
| A-09 | **Costo della CI su macOS** | **causa individuata, tre correzioni applicate; saldo da misurare al secondo run con la cache** | **LA CAUSA** (run #28, 2h 40m contro le 3h 13m del #27): **JUCE non condivide i moduli compilati fra target** — ogni consumatore ricompila `juce_gui_basics`, `juce_graphics`, `juce_audio_processors`… per conto proprio, e su macOS sono unity build in Objective-C++. Contate **45 unità JUCE in un solo run**: 14 per `Harmonizer`, **14 per `mode_switch_test`, 14 per `beta_gate_audio_test`**, 2 e 1 per gli altri. Prova diretta: l'avviso sullo splash screen di JUCE compare **tre volte in un minuto** (12:19:34, 12:19:47, 12:20:15). `beta_gate_audio_test` è **nato in s.37**: un terzo del problema l'ha creato quella sessione. ✅ **Applicate**: (1) le 5 sonde sono `EXCLUDE_FROM_ALL` — non possono girare in CI (vogliono i WAV di `SAMPLE TEST/`) ma venivano compilate a ogni run; (2) banchi e sonde su **una sola architettura** su macOS, con la lista ricavata da `BUILDSYSTEM_TARGETS` e non scritta a mano, così un banco nuovo eredita la regola da solo (una lista a mano ripeterebbe la causa di A-06); (3) **`ccache`** con `actions/cache`, launcher anche su OBJC/OBJCXX perché il costo sta nei `.mm`. ⚠️ **Limite accettato**: ccache si arrende **in silenzio** con più di un `-arch` per invocazione, quindi il **plugin universal non viene cachato** — vengono cachati i banchi, che grazie a (2) sono a una architettura sola: cioè esattamente la parte duplicata. **Il payoff arriva dal secondo run con la cache, non dal primo.** Un passo stampa il rapporto di colpi: se resta a zero, la strada è sbagliata. ❌ **Ancora aperte**: `paths-ignore` per i push di sola documentazione (**escluse `BETA-*.md`**, che entrano nel pacchetto) · **job macOS solo a richiesta**, che taglia quasi tutto ma riduce la copertura conquistata con D-23 — **decisione dell'utente**, A-06 nasceva proprio da buchi di copertura. **Scartata**: una libreria statica condivisa fra i due banchi pesanti — hanno definizioni di compilazione diverse (`HARMONIZER_BETA_*`). ⚠️ Ogni push fa partire un run completo: `[skip ci]` quando il commit non tocca il codice |
