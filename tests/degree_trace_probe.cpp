...
int main (int argc, char** argv)
{
    if (argc < 3)
    {
        std::printf ("Uso: degree_trace_probe <dry.wav> <tabella 12 celle> [root=0] [block=1024] [tIni tFin]\n");
        std::printf ("  es.: degree_trace_probe dry.wav \"-7,-2,-7,-2,-7,,,,,,,\" 0 1024\n");
        return 1;
    }
    ...
    while (done + block <= (int) mono.size())
    {
        ...
        if (inputIsStable)
        {
            heldNote = pitchLatch.update (continuousMidi, onsetThisBlock, block);
