...
**Immediato — riprendere il test in Ableton con ENTRAMBE le correzioni di sessione 11:**
1. Ricaricare "Harmonizer" (VST3 ricompilato in questa sessione, seconda volta).
2. **Ripetere di nuovo il test C→D→E cantato legato**: questa e' la terza verifica su questo stesso sintomo — le prime due (isteresi da sola) non erano sufficienti. Ogni nota nuova deve aggiornare l'armonizzazione secondo la tabella del preset in uso, senza bisogno di uno stacco netto. Se ANCORA non funziona, non assumere che sia lo stesso meccanismo: verificare con la label "Detected" se la nota mostrata segue correttamente (isolerebbe se il problema è ancora a monte, nel rilevamento) o se il problema e' altrove.
3. Provare anche un vibrato normale su una nota tenuta (non deve far sfarfallare l'armonizzazione) ed eventualmente un vibrato deliberatamente ampio (dovrebbe sentirsi l'alternanza fra le note vicine, comportamento voluto).
4. Se il comportamento non convince, due parametri distinti da tarare secondo il sintomo: soglia ±25 cent (`kHysteresisSemitones` in `PitchLatch.h`) se il problema e' di intonazione/quantizzazione; le soglie del gate in `OnsetDetector.cpp` (`-24_dB, -30_dB, -36_dB`) se il problema e' ancora "si ferma" nonostante l'isteresi (es. se il gate si chiude troppo facilmente durante un legato debole/piano).
5. **Osservare la label "Detected"** durante i test dei punti 2/5 originali (sample non riconosciuti da alcuni synth, primo attacco a "0 voci"): ancora da diagnosticare, vedi limiti noti sotto.
6. Poi proseguire con gli ascolti ancora non fatti: motore PSOLA (voicing a -12 semitoni e sotto, Fix/Move, cambio Stability — mappatura `{165,130,100,85,70}` Hz in `PsolaShifter.cpp` da tarare se serve), Formanti (costante `k=0.3` in `Voice.cpp` da tarare se serve), controllo MIDI CC con hardware/automazioni reali (FR-36/37), modalita' Play (setup PRD §3.4, verificare FR-27/28).
...
- **`signalPresent`/`onsetDetector.isGateOpen()` come proxy di "il performer sta ancora suonando" (sessione 11, continuazione) non ancora provato all'ascolto**: le soglie del gate (-24/-30/-36 dB, 10ms) sono quelle scelte in sessione 7 per il rilevamento onset, non specificamente tarate per questo secondo scopo — se un legato molto piano/debole chiude comunque il gate, e' il primo punto da rivedere.
- **Due bug distinti sullo stesso sintomo (canto legato) in questa sessione**: risolto uno alla volta, verificato che il sintomo persistesse prima di cercare il secondo. Non e' garantito che non ce ne sia un terzo — il prossimo test e' quello che lo dira'.
